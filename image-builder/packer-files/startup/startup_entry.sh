...
echo `date` "[liftr | startup_entry.sh] currentDir: $currentDir" | tee -a $logfile
echo pwd $(pwd) | tee -a $logfile
echo `date` "[liftr | startup_entry.sh] startup script starts" | tee -a $logfile
...
if [ ! -f /mnt/swapfile ]; then
  ...
  echo `date` "[liftr | startup_entry.sh] re-created swap file" | tee -a $logfile
fi
...
echo "=======================================================================================================================" | tee -a $logfile
echo `date` "[liftr | startup_entry.sh] start docker" | tee -a $logfile
sudo mkdir -p /mnt/containers
sudo /bin/bash $currentDir/start-service.sh docker

echo "=======================================================================================================================" | tee -a $logfile
echo `date` "[liftr | startup_entry.sh] Populate container environment variables from compute tags" | tee -a $logfile
sudo /bin/bash $currentDir/get-env-var-from-tags.sh

if [ $? -ne 0 ]; then
  # Report that this script failed
  echo `date` "[liftr | startup_entry.sh] FAILED populate container environment variables from compute tags" | tee -a $logfile
  exit 1
fi

echo "=======================================================================================================================" | tee -a $logfile
echo `date` "[liftr | startup_entry.sh] Start geneva components, i.e. mdsd, AzSecPack" | tee -a $logfile
...
echo `date` "[liftr | startup_entry.sh] docker ps:" | tee -a $logfile
sudo docker ps | tee -a $logfile
echo `date` "[liftr | startup_entry.sh] startup script ends" | tee -a $logfile