...
namespace Microsoft.Liftr.RPaaS.Tests
{
    public class MetaRPStorageClientTests
    {
        ...
        [Fact]
        public async Task Retries_on_patch_if_resource_write_permission_throws_401_Async()
        {
            using var handlerMock = new MetaRPMessageHandler();

            using var response1 = new HttpResponseMessage() { StatusCode = HttpStatusCode.Unauthorized };
            using var response2 = new HttpResponseMessage() { StatusCode = HttpStatusCode.Unauthorized };
            using var response3 = new HttpResponseMessage() { StatusCode = HttpStatusCode.Unauthorized };
            using var response4 = new HttpResponseMessage() { StatusCode = HttpStatusCode.OK };
            handlerMock.QueueResponse(response1);
            handlerMock.QueueResponse(response2);
            handlerMock.QueueResponse(response3);
            handlerMock.QueueResponse(response4);
            ...
            Assert.Equal(HttpStatusCode.OK, response.StatusCode);
            Assert.Equal(4, handlerMock.SendCalled);
        }

        [Fact]
        public async Task Retries_on_patch_if_resource_write_permission_throws_403_Async()
        {
            using var handlerMock = new MetaRPMessageHandler();

            using var response1 = new HttpResponseMessage() { StatusCode = HttpStatusCode.Forbidden };
            using var response2 = new HttpResponseMessage() { StatusCode = HttpStatusCode.Forbidden };
            using var response3 = new HttpResponseMessage() { StatusCode = HttpStatusCode.Forbidden };
            using var response4 = new HttpResponseMessage() { StatusCode = HttpStatusCode.OK };
            handlerMock.QueueResponse(response1);
            handlerMock.QueueResponse(response2);
            handlerMock.QueueResponse(response3);
        ...
        [Fact]
        public async Task Retries_on_get_if_resource_read_permission_throws_403_Async()
        {
            using var handlerMock = new MetaRPMessageHandler();
            var testResponse = new TestResource();
            var rawResponse = JsonConvert.SerializeObject(testResponse);

            using var response1 = new HttpResponseMessage() { StatusCode = HttpStatusCode.Forbidden };
            using var response2 = new HttpResponseMessage() { StatusCode = HttpStatusCode.Forbidden };
            using var response3 = new HttpResponseMessage() { StatusCode = HttpStatusCode.Forbidden };
            using var response4 = new HttpResponseMessage() { StatusCode = HttpStatusCode.OK, Content = new StringContent(rawResponse) };
            handlerMock.QueueResponse(response1);
            handlerMock.QueueResponse(response2);
            handlerMock.QueueResponse(response3);
            handlerMock.QueueResponse(response4);
            ...
            var response = await metaRpClient.GetResourceAsync<TestResource>("testresourceid", "testtenantid", Constants.ApiVersion);

            Assert.Equal(4, handlerMock.SendCalled);
        }

        [Fact]
        public async Task Retries_on_get_if_resource_read_permission_throws_401_Async()
        {
            using var handlerMock = new MetaRPMessageHandler();
            var testResponse = new TestResource();
            var rawResponse = JsonConvert.SerializeObject(testResponse);
