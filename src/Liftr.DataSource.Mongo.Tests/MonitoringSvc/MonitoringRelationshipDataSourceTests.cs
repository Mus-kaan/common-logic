...
namespace Microsoft.Liftr.DataSource.Mongo.Tests.MonitoringSvc
{
    public sealed class MonitoringRelationshipDataSourceTests : IDisposable
    {
        ...
        {
            await RunRelationshipTestAsync(_collectionScope.Collection);
        }

        public static async Task RunRelationshipTestAsync(IMongoCollection<MonitoringRelationship> collection)
        {
            var ts = new MockTimeSource();
            using var rateLimiter = new MongoWaitQueueRateLimiter(100, TestLogger.VoidLogger);
            IMonitoringRelationshipDataSource<MonitoringRelationship> s = new MonitoringRelationshipDataSource(collection, rateLimiter, TestLogger.VoidLogger, ts);
