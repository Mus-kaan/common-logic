        ...
        public async Task CanEventHubAsync()
        {
            using (var scope = new TestResourceGroupScope("ut-stor-", _output))
            {
                try
                {
                    ...
                    var eh = await az.GetOrCreateEventHubAsync(TestCommon.Location, scope.ResourceGroupName, namespaceName, hubName, TestCommon.Tags);
                    Assert.Equal(hubName, eh.Name);

                    // Second deployment will not fail.
                    await az.GetOrCreateEventHubAsync(TestCommon.Location, scope.ResourceGroupName, namespaceName, hubName, TestCommon.Tags);
                    ...
                    var eh2 = await az.GetOrCreateEventHubAsync(TestCommon.Location, scope.ResourceGroupName, namespaceName, hubName2, TestCommon.Tags);
